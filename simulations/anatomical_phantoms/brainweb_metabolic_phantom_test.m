...
kineticRates = [0, 0.04, 0.02; 0, 0, 0]; % no kPB, effectively
...
% visualize kTRANS
figure,
imagescn(k_trans(:,:,slices),[0 max(k_trans(:,:,slices),[],'all')]); colormap fire;

% visualize kinetic rates
figure,
imagescn(k_maps(:,:,slices,1),[0 max(k_maps(:,:,slices,1),[],'all')]); colormap fire;

figure, % picking an arbitrarily small number for the lower scale bound since k13 == 0
imagescn(k_maps(:,:,slices,2),[-1e-12 max(k_maps(:,:,slices,2),[],'all')]); colormap fire;