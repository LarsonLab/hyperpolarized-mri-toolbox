...
flips(1:2,1:N,1) = repmat([20;90]*pi/180,[1 N]);
...
noise_VIF = 0;  % same noise for all flip schedules
    ...
    [Mxy_ev Mz_ev Mxy_iv Mz_iv] = simulate_Nsite_perfused_voxel_model...
        (Mz0, [R1P R1L], [kPL 0], kve, vb, flips(:,:), TR, VIF_z*VIFscale);
    %     figure
    % subplot(221)
...
disp(['estimate vif from simulation: ',num2str(sum(S(1,:))/sum(VIF_xy(1,:)))]);
...
% hold on;
% plot(t,VIF_z,'--*')
% legend('simu pyr','simu lac','vif pyr','vif lac')
% ------------------------------------------------
% % old model
% ------------------------------------------------
disp('------------------------');
...
params_est.kPL = kPL_est;
% add IV function
[params_fit(:) Sfit2(1:2,1:size(S,2)) fitev fitvif] = fit_function(S(:,:), VIF_xy, TR, flips(:,:), params_fixed, params_est, [], plot_fits);
%%
disp(sprintf('Input R1 = %f (pyr) %f (lac), kPL = %f', R1P, R1L, kPL))
...
figure
plot(t,squeeze(Sfit2),'--o')
hold on;
plot(t,fitvif,'--*')
legend('fitting pyr','fitting lac','vif pyr','vif lac')
...
disp(['estimate vif from fitting: ',num2str(sum(Sfit2(1,:))/sum(VIF_xy(1,:)))]);
...
plot(t,fitev(1,:),'--*')
plot(t,fitvif(1,:),'--s')
legend('simulation','extra vascular','intra vascular')
title('pyruvate comparison')
subplot(122)
hold on;
