...
plot_flag=11; plot_fits = 1; plot_mag = 0; load_avg=1; save_flag=1; disp_1xlsx_flag=0;
...
noise_multiplyer=1.8; % multiplyer to the noise mean
	...
	for file_counter=1:files_num % loop for one xlsx file
		...
		if use_flag==1
			...
			% title([num2str(A_scaler),', normalized tumor, kpl: ',num2str(struct2array(params_fit1_t))])
			% fitting for control;
			
			% subplot(2,3,2)
			[params_fit1_c(:) sig_fit1_c(1:size(Mxy, 2))] = fit_function(Mxy(:, :,2), TR, flips( :, :), params_fixed_1, params_est_1, std_noise, plot_fits);
			% title(['normalized control, kpl: ',num2str(struct2array(params_fit1_c))])
			% fitting for noise;
...
%                     legend('original control','normalize control','without noise control')
% 			end
		end
