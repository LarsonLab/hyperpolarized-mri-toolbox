...
for file_index=1:files_num % loop for one mat file
	...
	for Dtype = 1:3 % tumor control noise
	    % no noise
	    [params_fit(:,Dtype) Sfit(1:2,1:size(Mxy,2),  Dtype)] = fit_function(Mxy(:,:,Dtype), VIF, TR, flips(:,:), params_fixed, params_est, [], plot_fits);
...
if plot_flag==1
	...
	plot(t, squeeze(Mxy(1,:,:)))
	hold on;
	plot(t, squeeze(Sfit(1,:,1:2)),':*')
	plot(t, squeeze(Snfit_mag(1,:,1:2)),'--s')
	...
	plot(t, squeeze(Sfit(2,:,1:2)),':*')
	plot(t, squeeze(Snfit_mag(2,:,1:2)),'--s')
	legend('ori tumor','ori cont','ori noise','no-noi tumor','no-noi cont','noi-f tumor','noi-f cont','Location','northeastoutside')
	title('Lac signals')
	...
	% hold on;
	% plot(t, squeeze((Sfit(1,:,1:2)-Mxy(1,:,1:2))./(Mxy(1,:,1:2))),':*')
	% plot(t, squeeze((Snfit_mag(1,:,1:2)-Mxy(1,:,1:2))./Mxy(1,:,1:2)),'--s')
	...
	% hold on;
	% plot(t, squeeze((Sfit(2,:,1:2)-Mxy(2,:,1:2))./Mxy(2,:,1:2)),':*')
	% plot(t, squeeze((Snfit_mag(2,:,1:2)-Mxy(2,:,1:2))./Mxy(2,:,1:2)),'--s')
	% legend('no-noi tumor','no-noi cont','noi-f tumor','noi-f cont','Location','northeastoutside')
