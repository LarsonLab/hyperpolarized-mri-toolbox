function [params_fit, Sfit, ufit, error_metrics] = fit_alanine_kinetics(S, TR, flips, params_fixed, params_est, noise_level, plot_flag)
...
params_default_est = [0.01, 0.01, ...
    1/30, 1/25, 1/25, ...
    0, 0, 0] ;%, ...
%    0.1, 0, 8];
params_default_lb = [-Inf, -Inf, ...
    1/50, 1/50, 1/50, ...
    -Inf, -Inf, -Inf,]; %, ...
%    0, -30, 0];
params_default_ub = [Inf, Inf, ...
    1/10, 1/10, 1/10, ...
    Inf, Inf, Inf]; %, ...
%    Inf 30 Inf];
...
function [Mz_all, u] = trajectories_inputless( params_fit, params_fixed, TR, Mzscale , Mz_alanine, Istart )
...
Mz_all(1,:) = Mz_alanine;
Mz_all(2,Istart) = S0_P;
Mz_all(3,Istart) = S0_L;
...
for It=Istart:N-1
    ...
    u(It) = ( Mz_alanine(It+1) - Mz_init(1)*exp(-k_substrate*TR) ) * k_substrate / (1 - exp(- k_substrate*TR));
    
    xstar = - inv(A)*[u(It),0,0].';
...
for It=Istart:-1:2
    ...
    % estimate input, assuming this is constant during TR interval
    % This calculation could be improved for noise stability?
    u(It-1) = ( Mz_alanine(It-1)*Mzscale(1,It-1) - Mz_init(1)*exp(-k_substrate*-TR) ) * k_substrate / (1 - exp(-k_substrate*-TR));
    
    xstar = - inv(A)*[u(It-1),0,0].';
