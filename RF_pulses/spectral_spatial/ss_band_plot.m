function ss_band_plot(f, a, d, fo, fs, f1, f2, sym)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% $Header: /home/adam/cvsroot/src/ss/ss_band_plot.m,v 1.6 2013/08/15 03:34:50 adam Exp $
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    hold on;
    for idx = 1:nf, 
	plot(f((idx*2-1):(idx*2))*fs/2+fo, [a(idx) a(idx)]);
	plot(f((idx*2-1):(idx*2))*fs/2+fo, [a(idx) a(idx)] + d(idx), '--');
	plot(f((idx*2-1):(idx*2))*fs/2+fo, [a(idx) a(idx)] - d(idx), ...
	     '--');
    end;
    grid;
	...
	for idx = 1:nf, 
	    ...
	    if (f_l ~= f_u), 
		plot([f_l f_u], [a(idx) a(idx)]);
		plot([f_l f_u], [a(idx) a(idx)] + d(idx), '--');
		plot([f_l f_u], [a(idx) a(idx)] - d(idx), '--');
	    end;
	end;
    end;
