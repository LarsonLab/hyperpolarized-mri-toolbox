function [params_fit, Sfit, ufit, error_metrics] = fit_pyr_kinetics_fixed_input(S, VIF, TR, flips, params_fixed, params_est, noise_level, plot_flag)
...
for i=1:size(Sreshape, 1)
    ...
    if any(Mxy(:) ~= 0)
        ...
        switch(fit_method)
            case 'ls'
                obj = @(var) difference_all(var, params_fixed, TR, Mzscale, Sscale, Mz, VIF_Mz, Nmets) ;  % perform least-squares in signal domain
            ...
            case 'ml'
                obj = @(var) negative_log_likelihood_rician(var, params_fixed, TR, Mzscale, Mz, VIF_Mz, noise_level.*(Sscale).^2, Nmets);
...
%         end
        Mzfit = trajectories_fixed_input(params_fit_vec(i,:), params_fixed, TR,  Mzscale, VIF_Mz) ;
...
%        ufit(i,:) = ufit(i,:)  .* Sscale(1, :);
        ...
        if plot_flag
            ...
            figure(99)
            subplot(2,1,1)
            plot(t, Mz(1:Nmets,:), t, Mzfit(1:Nmets,:),'--', t, VIF_Mz, 'k:') % ADD Rinj
            xlabel('time (s)')
            ylabel('state magnetization (au)')
            subplot(2,1,2)
            plot(t, Mxy(1:Nmets,:), t, squeeze(Sfit(i,1:Nmets,:)),'--', t, VIF, 'k:')
            xlabel('time (s)')
            ylabel('signal (au)')
...
function diff_products = difference_all(params_fit, params_fixed, TR, Mzscale, Sscale, Mz, VIF_Mz, Nmets)
global input_shape
...
Mzfit = trajectories_fixed_input(params_fit, params_fixed, TR,  Mzscale, VIF_Mz) ;
...
function [ l1 ] = negative_log_likelihood_rician(params_fit, params_fixed, TR, Mzscale, Mz, VIF_Mz, noise_level, Nmets)
...
function [Mz_all] = trajectories_fixed_input( params_fit, params_fixed, TR, Mzscale, VIF_Mz )
...
for It=Istart:N-1
    ...
    xstar = - inv(A)*[VIF_Mz(It)*Rinj,0,0,0].';
