function [params_fit, Sfit, ufit, objective_val] = fit_kPL_reverse(S, TR, flips, params_fixed, params_est, noise_level, plot_flag)
...
for i=1:size(S, 1)
    ...
    if any(y1(:) ~= 0)
        ...
        switch(fit_method)
            case 'ls'
                obj = @(var) (x2 - trajectories_frompyr_reverse(var, x1, Mzscale, params_fixed, TR)) .* Sscale(2,:);  % perform least-squares in signal domain
        ...
        [Sfit(i,:), ufit(i,:)] = trajectories_frompyr_reverse(params_fit_vec(i,:), x1, Mzscale, params_fixed, TR);
...
x2fit = trajectories_frompyr_reverse(params_fit, x1, Mzscale, params_fixed, TR);
