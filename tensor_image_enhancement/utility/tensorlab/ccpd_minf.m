function [U, output] = ccpd_minf(dataset, varargin)
%CCPD_MINF Coupled CPD by unconstrained nonlinear optimization.
%   U = CCPD_MINF(model) computes the coupled CPD defined by model. model is a
...
%   factors are allowed.
%   
%   U = CCPD_MINF(datasets, U0, cidx) computes the coupled CPD of the given
...
%
%   CCPD_MINF(model, options) and CCPD_MINF(datasets, U0, cidx, options) can be
...
%        cidx = {[1 2 3], [3 4 4]};
%       
%        Ures = ccpd_minf(data, U0, cidx, 'IsSymmetric', [true, false]);
